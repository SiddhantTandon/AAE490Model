function [Mbat, Mrotors, Mmotors,cap_batt, mass_panel, area_panel, radius_rotor, omega, P_mech_total, P_elec_total] = radiusOpt_2(solidity, tipMach, Cdp, mass_total, radius_vector, numProp, flightTime, V_batt, motor_eff,  sun_time, solar_flux, h, drone_vert_rate, v_cruise, beta, accel_climb, accel_forward)
...
%     Mrotors = 16.64;%20.332;%14.418;
    ...
    k = 1.15;            % Aerodynamic correction factor
...
for j=1:length(radius_vector)    % Iterate through many rotor radii 
    ...
    Mrotors(j) = 0.8*(t_blade*chord)*chord*rho_blade*radius_vector(j)*number_blades*numProp;
    ...
    [Time_climb_hr, P_climb, omega_climb(j), cap_batt_climb_accel, P_mech_climb_accel, P_elec_climb_accel, t_climb_accel] = Power_Climb(weight, rho_vert, radius_vector(j), a, tipMach, Cdp, solidity, drone_vert_rate, h, A_body, Cd_body, Idraw, V_batt, motor_eff, accel_climb, numProp);
